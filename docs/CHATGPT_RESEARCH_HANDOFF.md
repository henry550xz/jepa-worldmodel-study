...
## 2026-09-15 — supplied beta/rho hypothesis implemented and validated

User supplied independent AR(1) state/nuisance theory with nuisance magnitude beta and rho_s=0.9. Supersedes the prior unresolved-definition status. Implemented actual shared-encoder unit-variance latent prediction via stationary covariance/lag eigensolver, retaining prior provisional reference unchanged. Snapshot `6732a6d3ade3559a639c11a104c5dfaf2fe99d98`; CPU-only requested36-cell grid, five seeds,20k training/20k held-out pairs each, plus30 boundary cells per seed. Six tests passed.360/360 main-grid method/seed selections agree; at5% either side of the boundary all seeds agree for rho_n>=0.3; rho_n=0.1 shows finite-sample uncertainty. At beta4/rho_n0.5 state MSE Pixel0.99894 versus JEPA0.19002; at rho_n0.95 both approximately0.999, confirming nuisance failure. Observation prediction favors Pixel in the interesting regime.

Full equations, estimator distinction, metrics and limitations: [LINEAR_GAUSSIAN_THEORY_RESULTS.md](LINEAR_GAUSSIAN_THEORY_RESULTS.md). Authoritative artifacts: storage `artifacts/linear-gaussian-theory-20260915/` JSON and phase diagrams. No neural/robotics superiority claim. Theory blocker resolved; compute/information budget still needs review and full robotics runs remain unauthorized. No worker contact or GPU work. Current snapshot updated; prior evidence and historical findings preserved.

## 2026-09-15 — authorized four-arm mechanism queue launched

User “run it” authorized the previously recommended matched-window/update design. Kept architecture, frozen partitions, seed0, batch32 FP32, horizon3, consistency coefficient1, Gaussian regularizer0.5, evaluation settings unchanged; two epochs of52,215 updates each. Equal FLOPs are not claimed; conditioning intervention and actual costs must be reported. Added guarded full queue and full-profile closure evaluator, reusing matched training loop; exempted intentionally dormant Gaussian decoder from nonzero-gradient requirement. Fourteen tests and8-update full-loop Gaussian preflight passed (training, validation, checkpoint). Preflight evidence under worker `runs/mechanism-full-launch-preflight-98816c6/`, preserved.

Immutable snapshot `98816c638b4c2329079feffafde4090c0614cf3a`. Detached guarded worker queue `mechanism-seed0-20260915T2225Z`, initial PID488900. Run ID suffixes pixel/pixel_rollout/pixel_consistency/gaussian. Sequential training followed by full common evaluation and closure after each arm; stop on failure, preserve artifacts. Controller `jepa-mechanism-monitor.service` synchronizes evidence and SHA256-verifies final latest checkpoints without deletion. Initial Pixel training advancing, loss finite, GPU active;20GiB worker and13GiB controller data-volume headroom. No completed full-run scientific result yet. Next authorized action is supervise/reconcile this queue, not create another. Full completion requires validated evaluations and verified retention/reporting.

Launch follow-up: detached queue verified advancing after original SSH connection closed; controller retention observer completed initial synchronization. Measured201→284 updates/28.8s gives92.2windows/s, about10h Pixel training remaining, substantially slower than compute-only0.149s/update because of input/wait overhead. GPU sampled0–79%,13,364MiB VRAM. Other arms provisionally9–11h each if comparable input cost; total training37–43h plus evaluation time not yet measured for these new objectives. No throughput optimization or scientific setting change was made.

## 2026-09-16 — two-way throughput benchmark and authorized scheduling change

User requested parallel execution. Tested active Pixel plus disposable rollout model without changing frozen batch/architecture/seed/data/objectives. Pixel standalone121updates/40.761s; rollout standalone118/40.741s with Pixel briefly SIGSTOP-paused in memory; concurrent182 Pixel+173 rollout updates/61.171s after warmup. Rates94.99/92.68 standalone,95.21+90.50 concurrent windows/s; aggregate185.71,~1.98× equal-work sequential throughput. Driver VRAM23,173MiB, mean sampled concurrent GPU80.25%; no OOM, benchmark stopped and Pixel resumed without lost updates. Bounded benchmark snapshot `93500cf7c5ae392d22d201e329e2b5165e5d6ad1`; retained controller `runs/mechanism-pair-benchmark-20260916/`. Not a scientific result; pause/contended intervals must be accounted for in timing comparisons.

Replaced old sequential coordinator only (PID488900), preserving live Pixel train PID489052, with detached guarded two-slot coordinator PID257403, scheduling snapshot `90a7e7b156ca13b0d461b57558ef06d05568ef9f`. Original scientific training/evaluation SHA remains `98816c638b4c2329079feffafde4090c0614cf3a` for every arm. Same queue/run IDs; newly active rollout suffix pixel_rollout. First pair Pixel+rollout; then serialized evaluation; second pair consistency+Gaussian; then serialized evaluation. Failure pauses surviving trainers for diagnosis and stops advancement. No protocol/batch/update change. Second pair throughput is not yet measured; readiness memory peaks support two-way fit, requiring live verification. Existing controller observer remains active. Initial projected remaining training20–23h plus unknown evaluation cost. Current snapshot updated; no duplicate scientific runs.
