...
Run IDs share prefix `mechanism-seed0-20260915T2225Z-` with suffixes `pixel`, `pixel_rollout`, `pixel_consistency`, `gaussian`; future arm directories are created when reached. Launch command from committed worker snapshot: `bash scripts/study/worker/run_mechanisms.sh --authorized-full mechanism-seed0-20260915T2225Z` (ALREADY RUN; do not repeat). Worker data headroom20GiB at launch; controller research headroom13GiB. Fourteen tests and new8-update full-loop Gaussian preflight passed before launch. Initial Pixel loss finite/decreasing; GPU active. Initial wall-clock observation:201→284 updates over28.8s,2.88updates/s or92.2windows/s, approximately10h Pixel training remaining. Compute-only0.149s/update underestimates runtime because loading/waiting adds overhead; sampled GPU utilization0–79%, VRAM13,364MiB. Other arms provisionally9–11h each if similar loading overhead: total training roughly37–43h, with full evaluation time additional/unknown. Re-estimate from longer recent windows. Observer successfully retained initial state/logs; original launching SSH connection closed and detached queue still advancing.
