...
def main():
 ...
 s+='\n## Interpretation limits\n\nH1 repair is a readout change: no predicted latent has yet been fed back. Longer-horizon differences combine readout and feedback changes. Decode/reencode reduces block error at H1–16 but increases it at H32 in this cohort (only4 episodes); do not claim a complete repair or generalized superiority. Teacher forcing supplies fresh true history at every requested endpoint, so it diagnoses accumulation but is not a feasible open-loop substitute. Candidate-ranking and goal-success conclusions remain pending until all50 episodes per condition finish. The original physical metrics were checked against retained pilot outputs. See docs/REENCODE_CONTROL_PROTOCOL.md for settings, hashes and preserved startup failures.\n'
